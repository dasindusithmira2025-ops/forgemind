...
impl BrowserService {
    ...
    pub fn open(
        ...
        if let Some(view) = self.views.lock().unwrap().get(&key) {
            if let Some(webview) = self.app.get_webview(&view.webview_label) {
                ...
                if let Some(target) = target {
                    let mut current = view.current_url.lock().unwrap();
        ...
        let builder = WebviewBuilder::new(&label, initial)
            ...
            .on_page_load(move |webview, payload| {
                ...
                if url != "about:blank" {
                    if let Some(view) = load_views.lock().unwrap().get(&load_key) {
                        *view.current_url.lock().unwrap() = Some(url.clone());
                    }
                }
        ...
        self.views.lock().unwrap().insert(
            ...
            BrowserView {
                ...
                current_url: Mutex::new(target.map(|url| url.to_string())),
            },
        );
        Ok(())
    }
    ...
    fn with_webview<T>(
        ...
        let view = guard.get(&key).ok_or_else(no_view)?;
        let webview = self
            .app
    ...
    pub fn navigate(&self, window_label: &str, workspace_id: &str, url: &str) -> AppResult<()> {
        ...
        self.with_webview(window_label, workspace_id, |webview, view| {
            ...
            *view.current_url.lock().unwrap() = Some(parsed.to_string());
            Ok(())
        })
    }
    ...
    pub fn close(&self, window_label: &str, workspace_id: &str) -> AppResult<()> {
        let key = view_key(window_label, workspace_id);
        let removed = self.views.lock().unwrap().remove(&key);
    ...
    pub fn close_for_window(&self, window_label: &str) {
        ...
        let keys: Vec<String> = guard
            ...
            .collect();
        for key in keys {
            if let Some(view) = guard.remove(&key) {
                if let Some(webview) = self.app.get_webview(&view.webview_label) {
                    let _ = webview.close();
                }
            }
