...
export function sanitizeInspectedElement(raw: RawInspectedElement): SanitizedElement {
  const classNames = (raw.classNames ?? [])
  ...
  return {
    tag: clamp(raw.tag ?? 'unknown', 40).toLowerCase(),
    id: raw.id ? clamp(raw.id, 100) : undefined,
    classNames,
    attributes,
    text: raw.text ? redact(clamp(raw.text, INSPECT_LIMITS.text)) : undefined,
    accessibleName: raw.accessibleName ? redact(clamp(raw.accessibleName, INSPECT_LIMITS.accessibleName)) : undefined,
    selector: raw.selector ? clamp(raw.selector, INSPECT_LIMITS.selector) : undefined,
    rect: raw.rect
      ? {
      ...
      : undefined,
    domExcerpt: raw.domExcerpt ? redact(sanitizeDomExcerpt(raw.domExcerpt)) : undefined,
