...
describe('useBrowserSessionStore', () => {
  ...
  it('never persists credentials embedded in a browser URL', async () => {
    useBrowserSessionStore.getState().init('ws-secret')
    useBrowserSessionStore.getState().navigate('http://user:password@localhost:5173/private')
    await new Promise((resolve) => setTimeout(resolve, 260))

    const persisted = localStorage.getItem('paralith.browser.ws-secret') ?? ''
    expect(persisted).toContain('http://localhost:5173/private')
    expect(persisted).not.toContain('user')
    expect(persisted).not.toContain('password')
  })

  it('does not lose a pending save when switching workspaces quickly', async () => {
    useBrowserSessionStore.getState().init('ws-fast-a')
    useBrowserSessionStore.getState().navigate('http://localhost:3001/a')
    useBrowserSessionStore.getState().init('ws-fast-b')
    useBrowserSessionStore.getState().navigate('http://localhost:3002/b')
    await new Promise((resolve) => setTimeout(resolve, 260))

    expect(localStorage.getItem('paralith.browser.ws-fast-a')).toContain('localhost:3001')
    expect(localStorage.getItem('paralith.browser.ws-fast-b')).toContain('localhost:3002')
  })
