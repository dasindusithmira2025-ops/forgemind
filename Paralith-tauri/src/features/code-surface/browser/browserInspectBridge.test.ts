...
import { decodeInspectPayload, encodeInspectPayload, MAX_INSPECT_PAYLOAD_LENGTH } from './browserInspectBridge'
...
describe('inspect payload bridge', () => {
  ...
  it('rejects an oversized payload before decoding it', () => {
    expect(decodeInspectPayload('a'.repeat(MAX_INSPECT_PAYLOAD_LENGTH + 1))).toBeUndefined()
  })
})
