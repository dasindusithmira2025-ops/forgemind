...
export function SwarmOverview({ detail }: { detail: SwarmDetail }) {
  ...
  const archived = swarm.archived || swarm.lifecycle === 'archived'
  ...
  useEffect(() => {
    ...
    return () => { active = false }
  }, [archived, swarm.id, swarm.projectId])
    ...
    <div className="swarm-live-shell">
      <header className="swarm-live-header">
        ...
        <div className="swarm-live-actions">
          {!archived && (swarm.lifecycle === 'draft' ? <button type="button" className="button button-primary" disabled={Boolean(pendingOperation)} onClick={() => void start(swarm.id).catch(() => undefined)}><Play size={14} /> Validate &amp; launch</button> : swarm.lifecycle === 'paused' ? <button type="button" className="button button-secondary" disabled={Boolean(pendingOperation)} onClick={() => void resume(swarm.id).catch(() => undefined)}><Play size={14} /> Resume</button> : pausing ? <button type="button" className="button button-secondary" disabled><Pause size={14} /> Pausing</button> : active ? <button type="button" className="button button-ghost" disabled={Boolean(pendingOperation)} onClick={() => void pause(swarm.id).catch(() => undefined)}><Pause size={14} /> Pause</button> : null)}
          {!archived && (active || swarm.lifecycle === 'paused') && <button type="button" className="button button-ghost" disabled={Boolean(pendingOperation)} onClick={() => void stop(swarm.id, false).catch(() => undefined)}><CircleStop size={14} /> Stop</button>}
          {!archived && ['failed', 'cancelled'].includes(swarm.lifecycle) && <button type="button" className="button button-secondary" disabled={Boolean(pendingOperation)} onClick={() => void retry(swarm.id).catch(() => undefined)}><RotateCcw size={14} /> Retry failed work</button>}
      ...
      {!archived && swarm.lifecycle === 'decision_required' && swarm.decision ? <DecisionPanel detail={detail} onResolve={(choice) => resolveDecision(swarm.id, choice).catch(() => undefined)} /> : null}
      {!archived && (detail.attentionRequests ?? []).filter((request) => request.status === 'open').map((request) => <AttentionPanel key={request.id} request={request} pending={pendingOperation === 'attention'} onResolve={(response, approved) => resolveAttention(swarm.id, request.id, response, approved).catch(() => undefined)} />)}
      {swarm.lifecycle === 'ready_for_review' ? <div className="swarm-ready-banner"><CheckCircle2 size={17} /><div><strong>Ready for review</strong><span>{detail.reviews.length > 0 ? `${detail.reviews.length} independent Reviewer record${detail.reviews.length === 1 ? '' : 's'} persisted.` : 'Verification finished; human acceptance is still required.'}</span></div><button type="button" className="button button-ghost" onClick={() => openWork('changes')}>Review changes</button>{!archived ? <><button type="button" className="button button-ghost" onClick={() => { setTarget('@reviewer'); setInstruction('Run an additional manual check and attach the observed evidence.') }}>Run manual check</button><button type="button" className="button button-primary" disabled={Boolean(pendingOperation)} onClick={() => void accept(swarm.id).catch(() => undefined)}>Accept result</button></> : null}</div> : null}
      ...
      {!archived ? <ModelDefaults swarmId={swarm.id} projectId={swarm.projectId} agents={agents} /> : null}
      ...
      {!archived ? <div className="swarm-command-bar">
