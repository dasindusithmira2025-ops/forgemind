...
import { SWARM_PHASES, isActiveLifecycle, isPausableLifecycle, lifecycleLabel, lifecycleTone, phaseIndex, roleLabel, roleTarget, runtimeLabel } from './swarmPresentation'
...
export function SwarmOverview({ detail }: { detail: SwarmDetail }) {
  ...
  const immutable = swarm.lifecycle === 'archived'
  ...
  }, [immutable, swarm.id, swarm.projectId])
    ...
    <div className="swarm-live-shell">
      <header className="swarm-live-header">
        ...
        <div className="swarm-live-actions">
          {swarm.lifecycle === 'draft' ? <button type="button" className="button button-primary" disabled={Boolean(pendingOperation)} onClick={() => void start(swarm.id).catch(() => undefined)}><Play size={14} /> Validate &amp; launch</button> : swarm.lifecycle === 'paused' ? <button type="button" className="button button-secondary" disabled={Boolean(pendingOperation)} onClick={() => void resume(swarm.id).catch(() => undefined)}><Play size={14} /> Resume</button> : pausing ? <button type="button" className="button button-secondary" disabled><Pause size={14} /> Pausing</button> : canPause ? <button type="button" className="button button-ghost" disabled={Boolean(pendingOperation)} onClick={() => void pause(swarm.id).catch(() => undefined)}><Pause size={14} /> Pause</button> : null}
      ...
      {!immutable ? <ModelDefaults swarmId={swarm.id} projectId={swarm.projectId} agents={agents} /> : null}
      ...
      {!immutable ? <div className="swarm-command-bar">
...
function AgentInspector({ readOnly, agent, task, onClose, onMessage, onTerminal, onRetry, onValidate, onUpdate, onWork }: { readOnly?: boolean; agent: SwarmAgent; task?: SwarmTask; onClose: () => void; onMessage: () => void; onTerminal: () => void; onRetry: () => Promise<void>; onValidate: () => Promise<unknown>; onUpdate: (config: SwarmMemberModelConfig) => Promise<unknown>; onWork: (tab: WorkTab) => void }) {
