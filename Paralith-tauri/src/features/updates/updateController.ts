...
export function startUpdateController() {
  ...
  startPromise = (async () => {
    ...
    try {
      localListeners.push(await listen<UpdateStatus>('update-status', (event) => {
        useUpdateController.getState().setStatus(event.payload)
      }))
      if (generation !== controllerGeneration) return
      localListeners.push(await listen<UpdateDownloadProgress>('update-progress', (event) => {
        useUpdateController.getState().setProgress(event.payload)
      }))
      if (generation !== controllerGeneration) return
      unlisteners.push(...localListeners.splice(0))
    } catch (caught) {
      ...
      for (const unlisten of localListeners) unlisten()
    }
    try {
