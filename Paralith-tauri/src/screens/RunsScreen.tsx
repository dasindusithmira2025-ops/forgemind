...
/**
 * Project-level route hosting the canonical Run Engine surface. Uses the same full-screen shell
 * as the other project-level tools (own titlebar + Back), reached from the sidebar.
 *
 * Project scope is verified here rather than assumed from the URL: a Run reaches the filesystem
 * and Git, so a route pointing at a Project that is not the open one must be refused, not
 * rendered.
 */
export function RunsScreen() {
  ...
  return (
    <main className="repo-shell runs-screen">
      ...
      {error ? (
        ...
        <div className="centered-error">
          <ErrorNotice message="Open a Project before starting or viewing Runs." />
          <Button onClick={() => navigate('/')}>Open a Project</Button>
        </div>
      ) : activeProjectId !== projectId ? (
        <div className="centered-error">
          <ErrorNotice message="These Runs do not belong to the selected Project." />
