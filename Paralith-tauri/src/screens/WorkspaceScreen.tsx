...
import { AgentModeSurface } from '../features/agent-mode/AgentModeSurface'
import { ProductModeSwitch } from '../features/agent-mode/ProductModeSwitch'
import { useAgentModeStore } from '../features/agent-mode/agentModeStore'
...
export function WorkspaceScreen() {
  ...
  const productMode = useAgentModeStore((state) => state.mode)
  const setProductMode = useAgentModeStore((state) => state.setMode)
  const selectedOrganizationalAgent = useAgentModeStore((state) => state.snapshot.agents.find((agent) => agent.id === state.snapshot.productState.selectedAgentId))
  const codeOrigin = useAgentModeStore((state) => state.codeOrigin)
  const codeOriginAgent = useAgentModeStore((state) => state.snapshot.agents.find((agent) => agent.id === state.codeOrigin?.agentId))
  const openWorkInCode = useAgentModeStore((state) => state.openWorkInCode)
  const returnToAgent = useAgentModeStore((state) => state.returnToAgent)
  ...
  useEffect(() => {
    const handle = (event: KeyboardEvent) => {
      ...
      if (event.ctrlKey && event.key === ',') { event.preventDefault(); navigate('/settings'); return }
      if (event.ctrlKey && event.shiftKey && event.key.toLowerCase() === 'o') { event.preventDefault(); navigate('/'); return }
      // Code-surface shortcuts stop at the mode boundary. Splitting a pane or closing a terminal
      // from an Agent composer is the same class of bug as a terminal shortcut firing inside
      // Monaco; the application-level shortcuts above still work in both modes.
      if (productMode === 'agent') return
  ...
  // oxlint-disable-next-line react-hooks/exhaustive-deps
  }, [activePaneId, applyPreset, focusNextAttention, navigate, productMode, setActivePane, workspace, projectWorkspaces, switchWorkspace, toggleCollapse, togglePanel, openSourceControl])
  ...
  const renderPane = (paneId: string, ctx: RenderPaneContext) => {
    ...
    return <>
      <TerminalPane assignment={assignment} projectId={workspace.projectId} session={session} deferred={deferredPaneIds.includes(paneId)} active={ctx.active} maximized={ctx.maximized} settings={settings}
        onFocus={() => {
          setActivePane(paneId)
  ...
  return <AppShell className={`workspace-shell mode-${productMode} ${switchingWorkspaceId ? 'workspace-switching' : ''}`} sidebarOpen={!maximizedPaneId}
    ...
    statusBar={<><PaneBranchStatus projectId={project.id} directory={activePane?.workingDirectory} /><span className="status-path" title={project.rootPath}>{project.name}</span><span className="status-code-only">{running}/{workspace.panes.length} running</span><span className="status-code-only">{activePane?.title || 'No active pane'}</span><AiUsageStatusBar />{attentionQueue.length > 0 && <span className="status-alert">{attentionQueue.length} agent attention</span>}{Object.keys(paneErrors).some((id) => paneErrors[id]) && <span className="status-alert">Pane error</span>}</>}
  >
    <AgentModeSurface visible={productMode === 'agent'} project={project} workspace={workspace} onOpenCode={(target) => {
      // Opening work in Code focuses the *execution*: the workspace and pane its provider session
      // actually runs in, not the Project root. Nothing is created — the session already exists.
      const work = 'id' in target ? target : undefined
      if (work) openWorkInCode(work); else setProductMode('code')
      const targetWorkspaceId = work?.executionWorkspaceId ?? ('workspaceId' in target ? target.workspaceId : undefined)
      if (targetWorkspaceId && targetWorkspaceId !== workspace.id) navigate(`/workspace/${targetWorkspaceId}`)
    }} />
