...
import type {
  ...
  AgentRoutine,
  AgentSkill,
  SaveAgentRoutineInput,
  SaveAgentSkillInput,
} from './types'

export const native = {
  getAgentOrganization: () => invoke<AgentOrganizationSnapshot>('get_agent_organization'),
  createOrganizationalAgent: (input: CreateOrganizationalAgentInput) => invoke<OrganizationalAgent>('create_organizational_agent', { input }),
  createAgentConversation: (agentId: string, projectId: string | undefined, title: string) => invoke<AgentConversation>('create_agent_conversation', { agentId, projectId, title }),
  addAgentConversationEntry: (conversationId: string, body: string) => invoke<AgentConversationEntry>('add_agent_conversation_entry', { conversationId, body }),
  searchAgentHistory: (agentId: string, projectId: string, query: string) => invoke<AgentConversationEntry[]>('search_agent_history', { agentId, projectId, query }),
  createAgentDelegation: (input: CreateAgentDelegationInput) => invoke<AgentDelegation>('create_agent_delegation', { input }),
  startAgentWork: (input: StartAgentWorkInput) => invoke<AgentWork>('start_agent_work', { input }),
  cancelAgentWork: (workId: string) => invoke<void>('cancel_agent_work', { workId }),
  continueAgentWork: (workId: string, runtimeId?: string) => invoke<AgentWork>('continue_agent_work', { workId, runtimeId }),
  listAgentWorkEvents: (workId: string) => invoke<AgentWorkEvent[]>('list_agent_work_events', { workId }),
  saveAgentProductState: (mode: ProductMode, agentId?: string, conversationId?: string) => invoke<void>('save_agent_product_state', { mode, agentId, conversationId }),
  setOrganizationalAgentPinned: (agentId: string, pinned: boolean) => invoke<void>('set_organizational_agent_pinned', { agentId, pinned }),
  reorderOrganizationalAgents: (orderedIds: string[]) => invoke<void>('reorder_organizational_agents', { orderedIds }),
  reorderAgentConversations: (agentId: string, orderedIds: string[]) => invoke<void>('reorder_agent_conversations', { agentId, orderedIds }),
  listAgentRuntimes: () => invoke<AgentRuntimeOption[]>('list_agent_runtimes'),
  sendAgentMessage: (input: SendAgentMessageInput) => invoke<AgentConversationEntry>('send_agent_message', { input }),
  cancelAgentMessage: (entryId: string) => invoke<void>('cancel_agent_message', { entryId }),
  setAgentConversationRuntime: (conversationId: string, runtimeId?: string) => invoke<void>('set_agent_conversation_runtime', { conversationId, runtimeId }),
  setAgentIntelligencePreference: (agentId: string, preference: string) => invoke<void>('set_agent_intelligence_preference', { agentId, preference }),
  setAgentWorkspaceAccess: (agentId: string, projectId: string, access: AgentProjectAccess, workspaceId?: string) => invoke<AgentOrganizationSnapshot>('set_agent_workspace_access', { agentId, projectId, workspaceId, access }),
  listAgentCapabilities: (agentId: string) => invoke<AgentCapability[]>('list_agent_capabilities', { agentId }),
  setAgentCapability: (agentId: string, capability: string, decision: AgentCapabilityDecision) => invoke<AgentCapability[]>('set_agent_capability', { agentId, capability, decision }),
  listAgentApprovals: () => invoke<AgentApproval[]>('list_agent_approvals'),
  decideAgentApproval: (approvalId: string, approved: boolean, note?: string) => invoke<AgentApproval>('decide_agent_approval', { approvalId, approved, note }),
  listAgentSkills: () => invoke<AgentSkill[]>('list_agent_skills'),
  listAgentSkillAssignments: () => invoke<Array<[string, string]>>('list_agent_skill_assignments'),
  saveAgentSkill: (input: SaveAgentSkillInput) => invoke<AgentSkill>('save_agent_skill', { input }),
  deleteAgentSkill: (skillId: string) => invoke<void>('delete_agent_skill', { skillId }),
  setAgentSkillAssigned: (agentId: string, skillId: string, assigned: boolean) => invoke<void>('set_agent_skill_assigned', { agentId, skillId, assigned }),
  listAgentRoutines: () => invoke<AgentRoutine[]>('list_agent_routines'),
  saveAgentRoutine: (input: SaveAgentRoutineInput) => invoke<AgentRoutine>('save_agent_routine', { input }),
  deleteAgentRoutine: (routineId: string) => invoke<void>('delete_agent_routine', { routineId }),
  runAgentRoutineNow: (routineId: string) => invoke<AgentWork>('run_agent_routine_now', { routineId }),
