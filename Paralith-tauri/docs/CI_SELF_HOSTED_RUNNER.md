...
- removes all untracked and ignored files **except** package `node_modules` directories and
  `Paralith-tauri/src-tauri/target`;
- deletes `Paralith-tauri/.artifacts/`, and for releases
  `Paralith-tauri/src-tauri/target/release/bundle`, so no installer
...
> **Destructive.** The script deletes every untracked and ignored file except package dependency
> caches and `Paralith-tauri/src-tauri/target`. In a working tree that means uncommitted new work. It therefore
...
Remove-Item -Recurse -Force .\Paralith-tauri\.artifacts, .\Paralith-tauri\src-tauri\target\release\bundle -ErrorAction SilentlyContinue

# Full Rust rebuild (slow — reclaims ~75 GB)
Remove-Item -Recurse -Force .\Paralith-tauri\src-tauri\target
