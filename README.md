...
| [`corelith-web/`](corelith-web/README.md) | `corelith_web` | Legacy Corelith Technologies website (Next.js and Tailwind CSS). Still the deployed site. |
| [`corelith-site/`](corelith-site/README.md) | `corelith_site` | Rebuilt Corelith Technologies website (Next.js, Tailwind CSS, three.js). Not yet deployed. |
...
```powershell
Set-Location corelith-site
npm ci
npm run dev
```

`corelith-site` is a greenfield rebuild that runs alongside the legacy site rather than
replacing it. Cutover is a separate, explicit decision.
