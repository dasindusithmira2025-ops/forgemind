/**
 * Delivery verification for the gate film.
 *
 *   node scripts/verify-gate.mjs
 *
 * Successful encoding is not delivery. This asserts, per file, that the container is what was asked
 * for and — crucially — that the duration matches the cut length imported from `src/gate/script.ts`.
 * That import is the point: a re-timed sequence that has not been re-rendered fails here rather
 * than shipping, which is the failure mode a human reviewer is least likely to catch by watching.
 */
...
const { CUT_FRAMES, FPS } = await import(pathToFileURL(path.join(root, 'src/gate/script.ts')).href);
...
const EXPECTED = [
  { file: 'paralith-gate-film-4k.mp4', width: 3840, height: 2160, cut: 'master', audio: true },
  { file: 'paralith-gate-film-1080p.mp4', width: 1920, height: 1080, cut: 'master', audio: true },
  { file: 'paralith-gate-film-captioned.mp4', width: 1920, height: 1080, cut: 'master', audio: true },
  { file: 'paralith-gate-film-60s.mp4', width: 1920, height: 1080, cut: 'sixty', audio: true },
  { file: 'paralith-gate-film-30s.mp4', width: 1920, height: 1080, cut: 'thirty', audio: true },
  { file: 'paralith-gate-film-15s-teaser.mp4', width: 1920, height: 1080, cut: 'teaser', audio: true },
  { file: 'paralith-gate-hero-loop.mp4', width: 1920, height: 1080, cut: 'loop', audio: false },
];

const SIDECARS = [
  'paralith-gate-film-poster.png',
  'paralith-gate-film-captions.srt',
  'paralith-gate-film-captions.vtt',
  'paralith-gate-film-60s-captions.srt',
  'paralith-gate-film-30s-captions.srt',
  'paralith-gate-film-15s-captions.srt',
];
...
for (const entry of EXPECTED) {
  ...
  console.log(`ok  ${entry.file.padEnd(40)} ${video?.width}x${video?.height}  ${duration.toFixed(3)}s  ${size} MB${level}`);
}
...
console.log('\nall gate film deliveries verified');
