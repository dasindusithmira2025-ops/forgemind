...
```text
Project files + explicit read-only SQLite file selection
  -> discovery evidence
...
Explicit SQLite file selection ──read-only path──> DatabaseIntrospectionService
                                                   │
                                      DatabaseAdapterRegistry
                              prisma | drizzle | raw_sql | sqlite
                                                   │
...
4. Mutation is one SQLite transaction with a mandatory order: conditional `UPDATE database_designs ... WHERE head_revision_id=:expected AND revision_number=:expected`; require one affected row; on zero, re-read actual tokens and return `DATABASE_DESIGN_STALE_REVISION`; only then insert revision and operation rows. Any residual revision-number uniqueness error maps to the same typed error.
...
- **Observed**: V1 schema facts obtained only from an explicitly selected SQLite file opened read-only. Network Observed sources are Tier 2.
- **Proposed**: immutable design revision derived from a declared, observed, or earlier proposed base. New objects receive synthetic name-independent IDs that survive rename.

Layers share semantic object types and identity rules but never overwrite one another. V1 comparisons are stored as typed semantic diffs: declared-to-observed drift, declared-to-proposed delta, and design-to-design. Git-revision comparison is Tier 2.

## 6. Safety architecture

- Discovery performs static file analysis only and never auto-connects.
- V1 has no connection profiles or credential path. SQLite introspection opens only an explicitly selected Project file in read-only URI mode.
...
WP3 publishes debounced semantic canvas context through the WP4-owned `database_publish_canvas_state` command. WP4 stores only a bounded ephemeral Project/window/session projection and exposes it to the in-app orchestrator through `database.get_canvas_state` and `database.get_selection`.
...
4. **WP4 Builder** registers commands/events/capabilities, owns canvas-state publication, enforces execution mode, and implements the approved Prisma/raw-SQL pipeline with the contract allow-list and independent validation.
...
- No OS credential-store abstraction, PostgreSQL/MySQL driver, or connection pool was found. All network connection/profile commands and adapters are Tier 2 and absent in V1. The adapter trait remains the extension boundary.
...
- Git-revision schema comparison needs a safe file-at-revision reader and is Tier 2. V1 does not expose the mode.
...
- No external Claude Code/Codex capability bridge in V1. V1 agent operability means the existing in-app orchestrator drives the complete registered Database Studio capability set. `INTEGRATION-AUDIT.md`'s authenticated MCP design is accepted future direction only.
- No PostgreSQL/MySQL network introspection, OS credential store, connection-test stub, or network-introspection stub.
- No Drizzle native generation or Git-revision comparison in V1.
